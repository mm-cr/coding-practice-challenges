...
When a second (or subsequent) filter is applied, it again reduces by half the remaining pollution emitted after fitting
the existing filter(s).

For example, a factory that initially produces 6 units of pollution will generate 3 units with one filter, 1.5 units
with two filters and 0.75 units with three filters.
...
def solution(A):
    START = 0
    pollution: int = sum(A)
    target_reduction = pollution/2
    reduction_achieved = 0
    min_num_filters: int = 0

    while reduction_achieved < target_reduction:
        A.sort(reverse=True)
        A[START] = A[START]/2
        reduction_achieved += A[START]
        min_num_filters += 1

    return min_num_filters
